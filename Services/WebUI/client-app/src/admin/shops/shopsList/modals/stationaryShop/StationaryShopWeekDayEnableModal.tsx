...
interface Props {
    shop: StationaryShop,
    onSubmit: (weekDayToEnable: WeekDay) => void
}

export function StationaryShopWeekDayEnableModal({shop, onSubmit}: Props) {
    const init = {weekDay: ''}
            ...
            {({handleSubmit, handleChange, isValid, values}) => (
                <Form style={{width: '100%'}} onSubmit={handleSubmit} autoComplete='off'>
                    <Stack direction={'column'} spacing={2}
                        ...
                        <Typography textAlign={'center'} variant={'h4'}>Odblokuj dzień pracy</Typography>
                        <FormControl fullWidth required>
                            ...
                            >
                                {shop.weekSchedule.map(d =>
                                    d.isClosed && d.cachedOpenHours && <MenuItem key={d.weekDay.value} value={d.weekDay.value} >{d.weekDay.value}</MenuItem>
                                )}
                            </Select>
                        </FormControl>
